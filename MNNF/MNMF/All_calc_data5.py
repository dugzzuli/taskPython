...
def calDistance_two(x,y):

    dist= dt.distance(x, y)
    return dist
...
def cal_data_scale(data,distance=my_custom_norm_edu):
    ...
    for i in range(m):
        for j in range(m):
            if (i!=j):
                # calDistance_two
                distanceMat[i, j] = calDistance_two(distanceMat_stand[i, :], distanceMat_stand[j, :], distance);
    ...
    for dir_name in dirs:
        ...
        for file in range(len(files)):
            ...
            if(path.__contains__('n')):
                label[file,0]=1
            elif (path.__contains__('c')):
                label[file, 0] = 2
            elif (path.__contains__('o')):
                label[file, 0] = 3
            elif (path.__contains__('p')):
                label[file, 0] = 4
            elif (path.__contains__('t')):
                label[file, 0] = 5
